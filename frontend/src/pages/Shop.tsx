...
function Shop() {
  ...
  if (isError) return <div className='text-3xl text-gray-500 text-center mt-10'>Failed to fetch data!</div>
    ...
    <div className='min-h-screen pt-24 px-6 text-white flex gap-6'>
      ...
      <aside className='w-[260px] flex-shrink-0 flex flex-col items-center rounded-2xl bg-[#1b1321] shadow-xl border border-[#3f2e40] px-4 py-6'>
        ...
        <div className='w-full my-4'>
          <label className='text-sm text-white mb-1 block'>Max Price: ₹{price}</label>
          <input
            type="range"
            value={price}
            min={100}
      ...
      <div className='flex-1 flex flex-col'>
        ...
        <div className='mt-8 flex justify-center items-center gap-6'>
          ...
          <span className='text-gray-300 text-sm'>{page} of {data?.totalPage || 1}</span>
          <button
            className='text-white px-4 py-1 rounded hover:text-blue-400 disabled:opacity-30'
            disabled={!isNextPage}
            onClick={() => setPage(prev => prev + 1)}
